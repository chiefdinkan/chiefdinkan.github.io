...
"The only way to deal with fear is to face it head-on. - Sarah J. Maas",
"The greatest glory in living lies not in never falling, but in rising every time we fall. - Nelson Mandela",
"To live is to suffer, to survive is to find some meaning in the suffering. - Friedrich Nietzsche",
"Do not let your difficulties fill you with anxiety, after all, it is only in the darkest nights that stars shine more brightly. - Hasan Al-Basri",
...
"“The Western world has been thoroughly Judaized, and it’s time to do something about it.” - David Duke",
"“Nationalism is the only way to ensure that the future generations will have the chance to live in peace and harmony, in a country that is truly their own.” - Marine Le Pen",
...
if (localStorage.getItem("loadedQuotes")) {
  ...
  if (loadedQuotes.length === quotes.length) {
    localStorage.clear(); // Clear local storage
    loadedQuotes = []; // Reset loadedQuotes array
  }
} else {
  var loadedQuotes = [];
}
...
var unusedQuotes = quotes.filter(function(quote) {
  return loadedQuotes.indexOf(quote) === -1;
});
var randomQuote = unusedQuotes[Math.floor(Math.random() * unusedQuotes.length)];
